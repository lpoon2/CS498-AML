...
def inference(images):
  ...
  with tf.variable_scope('conv2') as scope:
    kernel = _variable_with_weight_decay('weights',
                                         ...
                                         stddev=1e-2,
                                         wd=None)
    conv = tf.nn.conv2d(conv1, kernel, [1, 4, 4, 1], padding='VALID')
    biases = _variable_on_cpu('biases', [64], tf.constant_initializer(0.0))
    pre_activation = tf.nn.bias_add(conv, biases)
  ...
  # norm1
  norm1 = tf.nn.lrn(pool1, 2, bias=2.0, alpha=0.001 / 9.0, beta=0.75,
                    name='norm1')
  ...
  # norm2
  norm2 = tf.nn.lrn(conv4, 2, bias=2.0, alpha=0.001 / 9.0, beta=0.75,
                    name='norm2')
  # pool2
  ...
  with tf.variable_scope('local4') as scope:
    ...
    reshape = tf.reshape(conv5, [images.get_shape().as_list()[0], -1])
