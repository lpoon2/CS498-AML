...
tf.app.flags.DEFINE_integer('batch_size', 384,
...
def inference(images):
  ...
  # norm2
  norm2 = tf.nn.lrn(conv6, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,
                    name='norm2')
  # pool2
  ...
  with tf.variable_scope('local5') as scope:
    ...
    local5 = tf.nn.relu(tf.matmul(dropout_1, weights) + biases, name=scope.name)
    _activation_summary(local5)

  dropout_2 = tf.layers.dropout(
          inputs=local5, rate=0.4)
  # linear layer(WX + b),
