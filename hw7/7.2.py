...
# three level of number of cluster
n_blob_count = [10, 20, 50]
...
def run_em(image_file):
    print 'Run KMeans and EM with '+image_file
    ...
    for n_blob in n_blob_count:

        # Initialization
        kmeans = KMeans(n_clusters=n_blob).fit(samples)
        ...
        output_file_name = image_file + '.' + str(n_blob) + 'segments.png'
        plt.savefig(output_file_name)
        plt.clf()
        print 'Saved '+output_file_name

def main():
    run_em('RobertMixed03.jpg')
    run_em('smallstrelitzia.jpg')
    run_em('smallsunset.jpg')
